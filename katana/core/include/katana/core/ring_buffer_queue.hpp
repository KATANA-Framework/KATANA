...
public:
    explicit ring_buffer_queue(size_t capacity = 1024) {
        ...
        buffer_ = static_cast<slot*>(::operator new(actual_capacity * sizeof(slot), static_cast<std::align_val_t>(alignof(slot))));
    ...
    ~ring_buffer_queue() {
        ...
        if (buffer_) {
            ...
            ::operator delete(buffer_, static_cast<std::align_val_t>(alignof(slot)));
        }
    }
