...
[[nodiscard]] field string_to_field(std::string_view name) noexcept;
[[nodiscard]] std::string_view field_to_string(field f) noexcept;
...
constexpr uint32_t fnv1a_hash(std::string_view str) noexcept {
    ...
    for (char c : str) {
        ...
        hash ^= static_cast<uint32_t>(static_cast<unsigned char>(lower));
        hash *= 16777619u;
    }
    return hash;
}

extern const std::array<field_entry, static_cast<size_t>(field::MAX_FIELD_VALUE)> field_lookup_table;
