...
public:
    struct options {
        ...
        in_memory_response_cache_executor::options cache{};
        in_memory_idempotency_executor::options idempotency{};
        in_memory_rate_limit_executor::options rate_limit{};
    };
    ...
    [[nodiscard]] in_memory_response_cache_executor& cache() noexcept { return cache_; }
    [[nodiscard]] in_memory_idempotency_executor& idempotency() noexcept { return idempotency_; }
    [[nodiscard]] in_memory_rate_limit_executor& rate_limit() noexcept { return rate_limit_; }

private:
    in_memory_response_cache_executor cache_;
    in_memory_idempotency_executor idempotency_;
    in_memory_rate_limit_executor rate_limit_;
...
public:
    ...
    redis_contract_policy_executor(redis_pool& pool, options opts)
        : pool_adapter_(std::make_unique<redis_pool_client>(pool)),
          ...
          rate_limit_store_(*pool_adapter_, opts.store),
          cache_((opts.cache.store = &cache_store_, std::move(opts.cache))),
    ...
    redis_contract_policy_executor(redis_kv_client& client, options opts)
        : cache_store_(client, opts.store), idempotency_store_(client, opts.store),
          rate_limit_store_(client, opts.store),
          cache_((opts.cache.store = &cache_store_, std::move(opts.cache))),
          idempotency_((opts.idempotency.store = &idempotency_store_, std::move(opts.idempotency))),
          rate_limit_((opts.rate_limit.store = &rate_limit_store_, std::move(opts.rate_limit))) {
        build_chain(opts.enable_cache, opts.enable_idempotency, opts.enable_rate_limit);
