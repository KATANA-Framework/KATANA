...
constexpr std::array<field_entry, static_cast<size_t>(field::MAX_FIELD_VALUE)> create_lookup_table() {
    std::array<field_entry, static_cast<size_t>(field::MAX_FIELD_VALUE)> result{};
    for (size_t i = 0; i < result.size(); ++i) {
        result[i] = {field_name_table[i], static_cast<field>(i), fnv1a_hash(field_name_table[i])};
    }
    return result;
}

const std::array<field_entry, static_cast<size_t>(field::MAX_FIELD_VALUE)> field_lookup_table = create_lookup_table();
...
field string_to_field(std::string_view name) noexcept {
    ...
    for (const auto& entry : detail::field_lookup_table) {
        if (entry.hash == hash) {
            bool match = true;
