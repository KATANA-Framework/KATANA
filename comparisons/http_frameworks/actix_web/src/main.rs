...
async fn compute_sum(nums: web::Json<Vec<f64>>) -> HttpResponse {
...
async fn list_items(state: web::Data<AppState>, query: web::Query<ListItemsQuery>) -> HttpResponse {
    let pool = match require_pool(state.get_ref()) {
        Ok(pool) => pool,
...
async fn get_item(state: web::Data<AppState>, id: web::Path<i64>) -> HttpResponse {
    let pool = match require_pool(state.get_ref()) {
        Ok(pool) => pool,
        Err(error) => return text_response(error.0, error.1),
    };
    ...
    let row = match client.query_opt(GET_ITEM_SQL, &[&id.into_inner()]).await {
        Ok(row) => row,
        Err(error) => return text_response(StatusCode::INTERNAL_SERVER_ERROR, error.to_string()),
    };
    match row {
...
async fn delete_item(state: web::Data<AppState>, id: web::Path<i64>) -> HttpResponse {
    let pool = match require_pool(state.get_ref()) {
        Ok(pool) => pool,
        Err(error) => return text_response(error.0, error.1),
    };
    ...
    let affected = match client.execute(DELETE_ITEM_SQL, &[&id.into_inner()]).await {
