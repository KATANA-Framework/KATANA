...
constexpr uint64_t HASH_LIST_PETS = hash_string("/pets");
...
inline katana::result<void> dispatch_list_pets(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    ...
    if (katana::http::infer_media_format(response_content_type) != katana::http::media_format::json) {
        out.assign_error(katana::problem_details::not_implemented("codec for Accept is not implemented"));
        return {};
    }
    ...
    auto p_X_Trace = req.headers.get("X-Trace");
    if (!p_X_Trace) { out = katana::http::response::error(katana::problem_details::bad_request("missing param X-Trace")); return {}; }
    auto X_Trace = p_X_Trace ? *p_X_Trace : std::string_view{};
...
// Dispatch for /pets
inline katana::result<void> dispatch_create_pet(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    ...
    if (auto validation_error = validate_PetCreateRequest(*parsed_body)) {
...
inline katana::result<void> dispatch_get_pet(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    ...
    int64_t petId = 0;
    {
        ...
        if (ec != std::errc() || ptr != p_petId->data() + p_petId->size()) { out = katana::http::response::error(katana::problem_details::bad_request("invalid path param petId")); return {}; }
    }
    // Set handler context for zero-boilerplate access
    katana::http::handler_context::scope context_scope(req, ctx);
