...
### 🚀 Оптимизации производительности (Tier 1) — 12.11.2025
* **Connection Pool**: Lock-free пул переиспользуемых соединений устраняет malloc/free в hot path (+15-25% RPS, -20% tail latency)
* **Zero-Copy Response**: Scatter-gather write напрямую из static буферов для простых ответов (+10-15% RPS, -10μs latency)
* **HTTP Parser Fast-Path**: Быстрая обработка `GET / HTTP/1.1` без полного state machine (+5-10% RPS, -15μs p50)
* **Cache Line Alignment**: 64-byte alignment структуры соединения снижает false sharing (+5-8% на многопоточной нагрузке)
* Совокупный эффект оптимизаций: ожидаемый прирост **20-40% RPS** при снижении tail latency

### Предыдущие обновления
* Переписан бенчмарк-харнес: прогрев, измерения и сбор хвостов происходят в таймбоксах, все ответы читаются полностью, а потоковые замеры делаются на фиксированной длительности.
* Последний прогон (08.11, 17:00 MSK, см. `BENCHMARK_RESULTS.md`) собрал 27k+ latency-семплов с p99 = 1.09 мс, IQR = 0.226 мс и 7.2k req/s на 8 потоках без артефактов на 64 коннектах.
* Реализован унифицированный `Timeout` helper с chunk-sleep и авто‑перезапуском для сервисных дедлайнов и мониторинга активности файловых дескрипторов.
* Колёсный таймер перестроен на плотные хэндлы и генерации: тик происходит строго по реальному времени, таймауты не «убегают» и гарантированно снимаются без гонок.
* Epoll-реактор переиспользует новую систему таймаутов, очищает дескрипторы после срабатывания и стабильно проходит unit/integration тесты.

### 📈 Качество бенчмарков (08.11, 17:00 MSK)

* 27 239 latency-семплов за основной прогон: p50 = 0.502 мс, p95 = 0.813 мс, p99 = 1.090 мс, p999 = 7.404 мс.
* IQR 0.226 мс и отсутствие «ступенек» в p95/p99/p999 указывают на корректные перцентили без квантования.
* Keep-alive и HTTP-parsing тесты собирают по 1500 стабильных семплов для каждого профиля запроса.
* Масштабирование по соединениям: 32→128 коннектов дают 13.6k→14.5k req/s без аномальной просадки на 64.
* Стресс-тест 4 потоками (5 с) выдаёт устойчивые 867 req/s при полном чтении ответов.
