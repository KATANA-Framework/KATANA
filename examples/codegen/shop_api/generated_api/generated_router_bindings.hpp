...
constexpr uint64_t HASH_CUSTOMER_REVENUE = hash_string("/shop/customers/revenue");
constexpr uint64_t HASH_TOP_PRODUCTS = hash_string("/shop/products/top");
constexpr uint64_t HASH_CATEGORY_STATS = hash_string("/shop/categories/stats");
constexpr uint64_t HASH_CREATE_ORDER = hash_string("/shop/orders");

// ============================================================
// Route Dispatch Functions
// ============================================================

// Dispatch for /shop/customers/revenue
inline katana::result<void> dispatch_customer_revenue(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    ...
    std::optional<std::string_view> p_country = std::nullopt;
    ...
    extract_query_params(req.uri, std::array{
        named_param_target{"country", &p_country},
    ...
    if (ctx.can_defer_response()) {
        if (auto* async_handler = dynamic_cast<async_api_handler*>(&handler)) {
            ...
            if (async_out) {
                if (async_handler->customer_revenue_async(country, limit, offset, async_out)) {
                    return {};
                }
                async_out.disarm();
    ...
    auto handler_result = handler.customer_revenue(country, limit, offset, out);
    if (!handler_result) {
...
// Dispatch for /shop/products/top
inline katana::result<void> dispatch_top_products(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    constexpr std::string_view kJsonContentType = "application/json";
    auto accept = req.headers.get(katana::http::field::accept);
    if (accept && !accept->empty() && *accept != "*/*" && *accept != kJsonContentType) {
        out.assign_error(katana::problem_details::not_acceptable("unsupported Accept header"));
    ...
    if (ctx.can_defer_response()) {
        if (auto* async_handler = dynamic_cast<async_api_handler*>(&handler)) {
            auto async_out = katana::http::async_response_writer(ctx.share_deferred_response(), kJsonContentType);
            if (async_out) {
                if (async_handler->top_products_async(limit, async_out)) {
                    return {};
                }
                async_out.disarm();
                ctx.reset_deferred_response();
    ...
    auto handler_result = handler.top_products(limit, out);
    if (!handler_result) {
        return std::unexpected(handler_result.error());
    }
    if (out.status != 204 && !out.body.empty() && !out.headers.get(katana::http::field::content_type)) {
...
// Dispatch for /shop/categories/stats
inline katana::result<void> dispatch_category_stats(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    constexpr std::string_view kJsonContentType = "application/json";
    auto accept = req.headers.get(katana::http::field::accept);
    if (accept && !accept->empty() && *accept != "*/*" && *accept != kJsonContentType) {
        out.assign_error(katana::problem_details::not_acceptable("unsupported Accept header"));
    ...
    if (ctx.can_defer_response()) {
        if (auto* async_handler = dynamic_cast<async_api_handler*>(&handler)) {
            auto async_out = katana::http::async_response_writer(ctx.share_deferred_response(), kJsonContentType);
            if (async_out) {
                if (async_handler->category_stats_async(async_out)) {
                    return {};
                }
                async_out.disarm();
                ctx.reset_deferred_response();
    ...
    auto handler_result = handler.category_stats(out);
    if (!handler_result) {
        return std::unexpected(handler_result.error());
    }
    if (out.status != 204 && !out.body.empty() && !out.headers.get(katana::http::field::content_type)) {
...
// Dispatch for /shop/orders/{id}
inline katana::result<void> dispatch_order_detail(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    constexpr std::string_view kJsonContentType = "application/json";
    auto accept = req.headers.get(katana::http::field::accept);
    if (accept && !accept->empty() && *accept != "*/*" && *accept != kJsonContentType) {
        out.assign_error(katana::problem_details::not_acceptable("unsupported Accept header"));
    ...
    if (ctx.can_defer_response()) {
        if (auto* async_handler = dynamic_cast<async_api_handler*>(&handler)) {
            auto async_out = katana::http::async_response_writer(ctx.share_deferred_response(), kJsonContentType);
            if (async_out) {
                if (async_handler->order_detail_async(id, async_out)) {
                    return {};
                }
                async_out.disarm();
                ctx.reset_deferred_response();
    ...
    auto handler_result = handler.order_detail(id, out);
    if (!handler_result) {
        return std::unexpected(handler_result.error());
    }
    if (out.status != 204 && !out.body.empty() && !out.headers.get(katana::http::field::content_type)) {
...
// Dispatch for /shop/orders
inline katana::result<void> dispatch_create_order(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    constexpr std::string_view kJsonContentType = "application/json";
    auto accept = req.headers.get(katana::http::field::accept);
    if (accept && !accept->empty() && *accept != "*/*" && *accept != kJsonContentType) {
        out.assign_error(katana::problem_details::not_acceptable("unsupported Accept header"));
    ...
    if (auto validation_error = validate_CreateOrderRequest(*parsed_body)) {
    ...
    if (ctx.can_defer_response()) {
        if (auto* async_handler = dynamic_cast<async_api_handler*>(&handler)) {
            auto async_out = katana::http::async_response_writer(ctx.share_deferred_response(), kJsonContentType);
            if (async_out) {
                if (async_handler->create_order_async(*parsed_body, async_out)) {
                    return {};
                }
                async_out.disarm();
                ctx.reset_deferred_response();
    ...
    auto handler_result = handler.create_order(*parsed_body, out);
    if (!handler_result) {
        return std::unexpected(handler_result.error());
    }
    if (out.status != 204 && !out.body.empty() && !out.headers.get(katana::http::field::content_type)) {
