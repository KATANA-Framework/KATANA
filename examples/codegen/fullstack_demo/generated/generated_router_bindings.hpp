...
constexpr uint64_t HASH_CREATE_NOTE = hash_string("/notes");
...
// Dispatch for /notes
inline katana::result<void> dispatch_create_note(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    ...
    if (auto validation_error = validate_CreateNoteRequest(*parsed_body)) {
    ...
    if (ctx.can_defer_response()) {
        if (auto* async_handler = dynamic_cast<async_api_handler*>(&handler)) {
            ...
            if (async_out) {
                if (async_handler->create_note_async(*parsed_body, async_out)) {
                    return {};
                }
                async_out.disarm();
    ...
    auto handler_result = handler.create_note(*parsed_body, out);
    if (!handler_result) {
...
// Dispatch for /notes
inline katana::result<void> dispatch_list_notes(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    constexpr std::string_view kJsonContentType = "application/json";
    auto accept = req.headers.get(katana::http::field::accept);
    if (accept && !accept->empty() && *accept != "*/*" && *accept != kJsonContentType) {
        out.assign_error(katana::problem_details::not_acceptable("unsupported Accept header"));
    ...
    std::optional<std::string_view> priority = std::nullopt;
    if (p_priority) priority = *p_priority;
    ...
    if (ctx.can_defer_response()) {
        if (auto* async_handler = dynamic_cast<async_api_handler*>(&handler)) {
            auto async_out = katana::http::async_response_writer(ctx.share_deferred_response(), kJsonContentType);
            if (async_out) {
                if (async_handler->list_notes_async(priority, limit, offset, async_out)) {
                    return {};
                }
                async_out.disarm();
                ctx.reset_deferred_response();
    ...
    auto handler_result = handler.list_notes(priority, limit, offset, out);
    if (!handler_result) {
        return std::unexpected(handler_result.error());
    }
    if (out.status != 204 && !out.body.empty() && !out.headers.get(katana::http::field::content_type)) {
...
// Dispatch for /notes/{id}
inline katana::result<void> dispatch_get_note(const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler, katana::http::response& out) {
    constexpr std::string_view kJsonContentType = "application/json";
    auto accept = req.headers.get(katana::http::field::accept);
    if (accept && !accept->empty() && *accept != "*/*" && *accept != kJsonContentType) {
        out.assign_error(katana::problem_details::not_acceptable("unsupported Accept header"));
    ...
    *p_id = katana::http_utils::percent_decode_view(*p_id, ctx.arena);
    ...
    if (ctx.can_defer_response()) {
        if (auto* async_handler = dynamic_cast<async_api_handler*>(&handler)) {
            auto async_out = katana::http::async_response_writer(ctx.share_deferred_response(), kJsonContentType);
            if (async_out) {
                if (async_handler->get_note_async(id, async_out)) {
                    return {};
                }
                async_out.disarm();
                ctx.reset_deferred_response();
    ...
    auto handler_result = handler.get_note(id, out);
    if (!handler_result) {
        return std::unexpected(handler_result.error());
    }
    if (out.status != 204 && !out.body.empty() && !out.headers.get(katana::http::field::content_type)) {
    ...
    explicit generated_router(api_handler& handler)
        ...
        katana::http::route_entry{katana::http::method::post,
                   ...
                   katana::http::handler_fn([handler_ptr = &handler](const katana::http::request& req, katana::http::request_context& ctx, katana::http::response& out) -> katana::result<void> {
                       return dispatch_create_note(req, ctx, *handler_ptr, out);
                   })
        ...
        katana::http::route_entry{katana::http::method::get,
                   katana::http::path_pattern::from_literal<"/notes">(),
                   katana::http::handler_fn([handler_ptr = &handler](const katana::http::request& req, katana::http::request_context& ctx, katana::http::response& out) -> katana::result<void> {
                       return dispatch_list_notes(req, ctx, *handler_ptr, out);
                   })
                   , katana::http::middleware_chain{}
                   , &route_policies_[1]
        },
        katana::http::route_entry{katana::http::method::get,
                   katana::http::path_pattern::from_literal<"/notes/{id}">(),
                   katana::http::handler_fn([handler_ptr = &handler](const katana::http::request& req, katana::http::request_context& ctx, katana::http::response& out) -> katana::result<void> {
                       return dispatch_get_note(req, ctx, *handler_ptr, out);
                   })
                   , katana::http::middleware_chain{}
                   , &route_policies_[2]
        },
