// Compute API comparison target using the generated standard router path only.
...
int main() {
    ...
    auto api_router = generated::make_router(handler);
    ...
    return http::server(api_router)
        ...
        .workers(workers)
        .on_start([&]() {
            std::cout << "Compute API router-only running on :" << port << " with " << workers
