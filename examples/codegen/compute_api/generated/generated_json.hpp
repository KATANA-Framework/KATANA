...
inline void serialize_compute_sum_request_array_into(const arena_vector<compute_sum_request>& arr, std::string& json) {
    json.push_back('[');
    for (size_t i = 0; i < arr.size(); ++i) {
        if (i > 0) json.push_back(',');
        serialize_compute_sum_request_into(arr[i], json);
...
inline void serialize_compute_sum_response_array_into(const arena_vector<compute_sum_response>& arr, std::string& json) {
    json.push_back('[');
    for (size_t i = 0; i < arr.size(); ++i) {
        if (i > 0) json.push_back(',');
        serialize_compute_sum_response_into(arr[i], json);
