...
// Inline dispatch for /text/uppercase
inline katana::result<katana::http::response> dispatch_text_uppercase(
    ...
    auto generated_response = handler.text_uppercase(*parsed_body);
...
// Inline dispatch for /text/lowercase
inline katana::result<katana::http::response> dispatch_text_lowercase(
    const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler) {
    auto negotiated_response = negotiate_response_type(req, route_1_produces);
    if (!negotiated_response) {
        return katana::http::response::error(
            katana::problem_details::not_acceptable("unsupported Accept header"));
    }
    ...
    // Set handler context for zero-boilerplate access
    katana::http::handler_context::scope context_scope(req, ctx);
    auto generated_response = handler.text_lowercase(*parsed_body);
    if (negotiated_response && !generated_response.headers.get(katana::http::field::content_type)) {
        generated_response.set_header("Content-Type", *negotiated_response);
    }
    return generated_response;
...
// Inline dispatch for /text/reverse
inline katana::result<katana::http::response> dispatch_text_reverse(
    const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler) {
    auto negotiated_response = negotiate_response_type(req, route_2_produces);
    if (!negotiated_response) {
        return katana::http::response::error(
            katana::problem_details::not_acceptable("unsupported Accept header"));
    }
    ...
    // Set handler context for zero-boilerplate access
    katana::http::handler_context::scope context_scope(req, ctx);
    auto generated_response = handler.text_reverse(*parsed_body);
    if (negotiated_response && !generated_response.headers.get(katana::http::field::content_type)) {
        generated_response.set_header("Content-Type", *negotiated_response);
    }
    return generated_response;
...
// Inline dispatch for /text/stats
inline katana::result<katana::http::response> dispatch_text_stats(
    const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler) {
    auto negotiated_response = negotiate_response_type(req, route_3_produces);
    if (!negotiated_response) {
        return katana::http::response::error(
            katana::problem_details::not_acceptable("unsupported Accept header"));
    }
    ...
    // Set handler context for zero-boilerplate access
    katana::http::handler_context::scope context_scope(req, ctx);
    auto generated_response = handler.text_stats(*parsed_body);
    if (negotiated_response && !generated_response.headers.get(katana::http::field::content_type)) {
        generated_response.set_header("Content-Type", *negotiated_response);
    }
    return generated_response;
...
// Inline dispatch for /text/transform
inline katana::result<katana::http::response> dispatch_text_transform(
    const katana::http::request& req, katana::http::request_context& ctx, api_handler& handler) {
    auto negotiated_response = negotiate_response_type(req, route_4_produces);
    if (!negotiated_response) {
        return katana::http::response::error(
            katana::problem_details::not_acceptable("unsupported Accept header"));
    }
    ...
    // Set handler context for zero-boilerplate access
    katana::http::handler_context::scope context_scope(req, ctx);
    auto generated_response = handler.text_transform(*parsed_body);
    if (negotiated_response && !generated_response.headers.get(katana::http::field::content_type)) {
        generated_response.set_header("Content-Type", *negotiated_response);
    }
    return generated_response;
        ...
        route_entry{
            ...
            handler_fn(
                [&handler](const katana::http::request& req, katana::http::request_context& ctx)
                    ...
                    auto matched_ct = find_content_type(
                        req.headers.get(katana::http::field::content_type), route_1_consumes);
                    if (!matched_ct)
                        return katana::http::response::error(
                            katana::problem_details::unsupported_media_type(
                                "unsupported Content-Type"));
        ...
        route_entry{
            ...
            handler_fn(
                [&handler](const katana::http::request& req, katana::http::request_context& ctx)
                    ...
                    auto matched_ct = find_content_type(
                        req.headers.get(katana::http::field::content_type), route_2_consumes);
                    if (!matched_ct)
                        return katana::http::response::error(
                            katana::problem_details::unsupported_media_type(
                                "unsupported Content-Type"));
        ...
        route_entry{
            ...
            handler_fn(
                [&handler](const katana::http::request& req, katana::http::request_context& ctx)
                    ...
                    auto matched_ct = find_content_type(
                        req.headers.get(katana::http::field::content_type), route_3_consumes);
                    if (!matched_ct)
                        return katana::http::response::error(
                            katana::problem_details::unsupported_media_type(
                                "unsupported Content-Type"));
        ...
        route_entry{
            ...
            handler_fn(
                [&handler](const katana::http::request& req, katana::http::request_context& ctx)
                    ...
                    auto matched_ct = find_content_type(
                        req.headers.get(katana::http::field::content_type), route_4_consumes);
                    if (!matched_ct)
                        return katana::http::response::error(
                            katana::problem_details::unsupported_media_type(
                                "unsupported Content-Type"));
...
public:
    ...
    katana::result<katana::http::response> operator()(const katana::http::request& req,
        ...
        case HASH_TEXT_UPPERCASE:
            if (path == "/text/uppercase" && req.http_method == katana::http::method::post) {
        ...
        case HASH_TEXT_LOWERCASE:
            if (path == "/text/lowercase" && req.http_method == katana::http::method::post) {
                // Hash matched, path matched, method matched - inline dispatch!
                return dispatch_text_lowercase(req, ctx, handler_);
            }
            break;
        case HASH_TEXT_REVERSE:
            if (path == "/text/reverse" && req.http_method == katana::http::method::post) {
                // Hash matched, path matched, method matched - inline dispatch!
                return dispatch_text_reverse(req, ctx, handler_);
            }
            break;
        case HASH_TEXT_STATS:
            if (path == "/text/stats" && req.http_method == katana::http::method::post) {
                // Hash matched, path matched, method matched - inline dispatch!
                return dispatch_text_stats(req, ctx, handler_);
            }
            break;
        case HASH_TEXT_TRANSFORM:
            if (path == "/text/transform" && req.http_method == katana::http::method::post) {
                // Hash matched, path matched, method matched - inline dispatch!
