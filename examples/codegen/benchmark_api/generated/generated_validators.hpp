...
[[nodiscard]] inline std::optional<validation_error> validate_RegisterRequest(const RegisterRequest& obj) {
    ...
    if (!obj.username.empty() && obj.username.size() < RegisterRequest::metadata::USERNAME_MIN_LENGTH) {
    ...
    if (!obj.username.empty()) {
        ...
        for (unsigned char ch_ : pv_) {
            if (!std::isalnum(ch_) && ch_ != '_' && ch_ != '-') { pattern_ok_ = false; break; }
        }
        if (!pattern_ok_) {
    ...
    if (!obj.password.empty() && obj.password.size() < RegisterRequest::metadata::PASSWORD_MIN_LENGTH) {
...
[[nodiscard]] inline std::optional<validation_error> validate_CreateItemRequest(const CreateItemRequest& obj) {
    ...
    if (obj.description && obj.description->size() > CreateItemRequest::metadata::DESCRIPTION_MAX_LENGTH) {
...
[[nodiscard]] inline std::optional<validation_error> validate_UpdateItemRequest(const UpdateItemRequest& obj) {
    if (obj.name && !obj.name->empty() && obj.name->size() < UpdateItemRequest::metadata::NAME_MIN_LENGTH) {
    ...
    if (obj.description && obj.description->size() > UpdateItemRequest::metadata::DESCRIPTION_MAX_LENGTH) {
